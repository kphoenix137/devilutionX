...
void PackPlayer(PlayerPack &packed, const Player &player)
{
	...
	for (int i = 0; i < MAX_SPELLS - 15; i++) // Should be MAX_SPELLS but set to 37 to make save games compatible
		packed.pSplLvl[i] = player._pSplLvl[i];
	for (int i = MAX_SPELLS - 15; i < MAX_SPELLS - 5; i++)
		packed.pSplLvl2[i - MAX_SPELLS - 15] = player._pSplLvl[i];
...
void UnPackPlayer(const PlayerPack &packed, Player &player)
{
	...
	for (int i = 0; i < MAX_SPELLS - 15; i++) // Should be MAX_SPELLS but set to 36 to make save games compatible
		player._pSplLvl[i] = packed.pSplLvl[i];
	for (int i = MAX_SPELLS - 15; i < MAX_SPELLS - 5; i++)
		player._pSplLvl[i] = packed.pSplLvl2[i - MAX_SPELLS - 15];
