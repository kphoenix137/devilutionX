...
void LoadMonster(LoadHelper *file, Monster &monster)
{
	...
	if (monster.isPlayerMinion()) // Don't skip for golems
		...
		file->Skip(2); // Skip hit as it's already initialized
	monster.minDamage = file->NextLE<uint16_t>();
	monster.maxDamage = file->NextLE<uint16_t>();
	file->Skip(2); // Skip toHitSpecial as it's already initialized
...
void SaveMonster(SaveHelper *file, Monster &monster)
{
	...
	file->WriteLE<uint16_t>(static_cast<uint16_t>(std::min<uint16_t>(monster.toHit, std::numeric_limits<uint16_t>::max()))); // For backwards compatibility
	file->WriteLE<uint16_t>(monster.minDamage);
	file->WriteLE<uint16_t>(monster.maxDamage);
	file->WriteLE<uint16_t>(static_cast<uint16_t>(std::min<uint16_t>(monster.toHitSpecial(sgGameInitInfo.nDifficulty), std::numeric_limits<uint16_t>::max()))); // For backwards compatibility
