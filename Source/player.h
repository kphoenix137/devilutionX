...
struct Player {
	...
	{
		if (_pClass == HeroClass::Barbarian && item._iLoc == ItemEquipType::TwoHand && IsAnyOf(item._itype, ItemType::Sword, ItemType::Mace))
...
void CheckPlrSpell(bool isShiftHeld, SpellID spellID = MyPlayer->_pRSpell, spell_type spellType = MyPlayer->_pRSplType);
