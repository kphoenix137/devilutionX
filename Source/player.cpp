...
const int PlrGFXAnimLens[enum_size<HeroClass>::value][11] = {
	// [0] Idle frameset, [1] attacking frameset, [2] walking frameset, [3] blocking speed,
	// [4] Death frameset, [5] magic/casting frameset, [6] hit recovery speed, [7] idle frameset in town,
	// [8] walking frameset in town, [9] single-handed attack speed, [10] spell casting speed
	// Note: speeds are multiplied by 0.05 to get actual time in seconds.
	// Frameset values must be larger than speeds to function correctly
...
/** Overrides the frame of each animation for which an action is triggered per weapon type, for each player class. */
const int PlrGFXAFrames[enum_size<HeroClass>::value][7] = {
	// Unarmed, UnarmedShield, Bow, Axe, Staff, Mace, MaceShield
	{		16,			   16,  16,  20,    16,   16,		  16 },
	{		18,			   18,  12,  22,    16,   18,		  18 },
	{		20,			   16,  20,  24,    16,   16,		  16 },
	{		12,			   12,  20,  23,    13,   16,		  16 },
	{		18,			   18,  12,  22,    16,   18,		  18 },
	{		16,			   16,  16,  20,    16,   16,		  16 },
};
/** Overrides the frame of each animation for which an action is triggered, for each player class. */
const int PlrGFXAFNum[enum_size<HeroClass>::value][7] = {
	// Unarmed, UnarmedShield, Bow, Axe, Staff, Mace, MaceShield
	{		 9,             9,  11,  10,    11,    9,          9 },
	{	    10,			   10,   7,  13,    11,   10,         10 },
	{		12,				9,  16,  16,	12,   12,		  12 },
	{		 7,				7,  14,  14,     8,   12,		  12 },
	{		10,			   10,  11,  13,    11,   10,         10 },
	{		 9,				9,  11,   8,    11,    8,          8 },
};
// clang-format on
...
void SetPlrAnims(Player &player)
{
	...
	if (leveltype == DTYPE_TOWN) {
		...
		player._pAFrames = PlrGFXAnimLens[static_cast<std::size_t>(pc)][1]; // default value
		player._pHFrames = PlrGFXAnimLens[static_cast<std::size_t>(pc)][6];
		player._pSFrames = PlrGFXAnimLens[static_cast<std::size_t>(pc)][5];
		player._pDFrames = PlrGFXAnimLens[static_cast<std::size_t>(pc)][4];
		player._pBFrames = PlrGFXAnimLens[static_cast<std::size_t>(pc)][3];
		player._pAFNum = PlrGFXAnimLens[static_cast<std::size_t>(pc)][9];	// default value
	}
	...
	case PlayerWeaponGraphic::UnarmedShield:
		player._pAFrames = PlrGFXAFrames[static_cast<std::size_t>(pc)][1];
		player._pAFNum = PlrGFXAFNum[static_cast<std::size_t>(pc)][1];
		break;
